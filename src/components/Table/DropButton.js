...
const DropButton = ({
	...
	const onClickMaxTypeLimited = useCallback(
		(select, dataRight, leftTableKey) => {
			// 이미 선택된 데이터
			let preDataType = [];
			if (dataRight) {
				preDataType = dataRight.map((v) => {
					return v.type;
				});
			}
			...
			// 새로 선택된 데이터
			const checkDataType =
				select[leftTableKey] && select[leftTableKey].map((v) => v.type);
	...
	const connectPolicyToRole = useCallback(
		(select, selectId, dataRight, leftTableKey) => {
			rightDataIds &&
				select &&
				onClickMaxTypeLimited(
					select,
					dataRight,
					leftTableKey,
					maxCount,
	...
	const connectUserGroupToRole = useCallback(
		(selectId, rightDataIds) => {
			rightDataIds &&
				selectId &&
				onClickMaxNumberLimited(selectId, rightDataIds) &&
				setRightDataIds &&
				setRightDataIds(rightDataIds.concat(selectId));
		},
		[usage, maxCount],
	...
	const onClickLeftDropButton = useCallback(() => {
		...
		const CONNECT_POLICY_TO_ROLE_EXCLUDE =
			tableKeys.roles.add.policies.exclude;
		const CONNECT_POLICY_TO_ROLE_INCLUDE =
		...
		const CONNECT_GROUPS_TO_ROLE_EXCLUDE =
			tableKeys.roles.add.groups.exclude;
		const CONNECT_GROUPs_TO_ROLE_INCLUDE =
			tableKeys.roles.add.groups.include;

		if (
			leftTableKey === CONNECT_POLICY_TO_ROLE_EXCLUDE &&
			rightTableKey === CONNECT_POLICY_TO_ROLE_INCLUDE
		) {
			connectPolicyToRole(
				select,
				selectId,
			...
			(leftTableKey === CONNECT_USER_TO_ROLE_EXCLUDE &&
				rightTableKey === CONNECT_USER_TO_ROLE_INCLUDE) ||
			(leftTableKey === CONNECT_GROUPS_TO_ROLE_EXCLUDE &&
				rightTableKey === CONNECT_GROUPs_TO_ROLE_INCLUDE)
		) {
